...
class Portfolio:
    def __init__(self, assets: list[str], weights: list[int], plot_always=False, plot_marker='o'):
        ...
        self.stat_var = -1
        self.stat_sharpe = -1
        self.pack_format = 'ddddd' + 'B'*len(self.weights)
    ...
    def serialize(self):
        return struct.pack(self.pack_format, self.stat_gain, self.stat_stdev, self.stat_cagr, self.stat_var, self.stat_sharpe, *self.weights)
        return pickle.dumps([
            self.stat_gain,
            self.stat_stdev,
            self.stat_cagr,
